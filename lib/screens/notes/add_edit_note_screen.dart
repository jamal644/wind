...
class _AddEditNoteScreenState extends State<AddEditNoteScreen>
  ...
  Future<void> _handleBack() async {
    // Always allow back navigation, even if saving
    if (!_isSaving) {
      // If not saving, just pop the route
      if (mounted) {
        Navigator.of(context).pop();
      }
    } else {
      // If saving, show a message but still allow back navigation
      if (mounted) {
      ...
      // Still allow back navigation even during save
      if (mounted) {
        Navigator.of(context).pop();
      }
    }
  ...
  Future<void> _saveNote() async {
    ...
    if (_isSaving) {
      ...
      // Instead of returning, we'll show a message to the user
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Saving in progress, please wait...'),
            duration: Duration(seconds: 2),
          ),
        );
      }
    ...
    try {
      ...
      if (mounted) {
        // Ensure any pending frames are rendered before popping
  ...
  void dispose() {
    ...
    // Ensure any pending frames are processed
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _debounceTimer?.cancel();
    });
